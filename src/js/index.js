...
const editar = ()=>{
 for (let i = 0; i < $('.elem-cargos').length; i++) {
  $('.elem-cargos').eq(i).click(()=>{
   ...
   const verificar = $('#txtCargo').val() !== $('.elem-cargos').eq(i).text() && $('.elem-cargos').eq(i).attr("estado") == "false" ? attr : attrdefault;
...
$(document).ready(()=>{
 ...
 listarCargos();
 $("#crearCargos").click(()=>{
  const lanzarAccion = $("#crearCargos").text() == "Crear" ? crearCargos : actualizarCargos; 
  lanzarAccion();
 });
});