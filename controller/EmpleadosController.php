...
class EmpleadosController extends ControladorBase{
 ...
 public function editar(){
  ...
  if(isset($_SESSION['usuario'])){
   if(isset($_POST['cedula']) && isset($_POST['nombre']) && isset($_POST['apellido'])){  
     $emp = new Empleados("empleados");
     $emp->actualizar($_POST["id"],$_POST["cedula"],$_POST["nombre"],$_POST["apellido"],$_POST["edad"],$_POST["sexo"],$_POST["cargo"]); 
   }
  }else{
   $this->redirect('login','validacionUsuario');
  }
 ...
 public function consultar(){
  ...
  if(isset($_SESSION['usuario'])){
   ...
   if(isset($_POST['columna']) && isset($_POST['dato'])){
    echo json_encode($emp->consultaUnida("cargo",$_POST['columna'],$_POST['dato']));
   }else{
    echo json_encode($emp->innerGetAll("cargo"));
   }
  }else {
 ...
 public function consultarPorId(){
  ...
  if(isset($_SESSION['usuario'])){
   if(isset($_POST['id'])){
    $emp = new Empleados("empleados");
    // echo json_encode($emp->getById($_POST['id']));
    echo json_encode($emp->consultaIds($_POST['id']));
   }
  }else {
   $this->redirect('login','validacionUsuario');
  }
